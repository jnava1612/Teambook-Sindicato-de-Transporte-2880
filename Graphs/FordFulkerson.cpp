// This algorithm solves the max flow problem in a directed graph in O(max_flow * E) 

// Here the graph is represented by an adjacency matrix, but it can be easily changed to an adjacency list

// The algorithm is based on the push-relabel algorithm, which is a variant of the relabel-to-front algorithm
...
int fordFulkerson(int graph[V][V], int s, int t)
{
    ...
    int rGraph[V]
              [V]; // Residual graph where rGraph[i][j] indicates residual capacity of edge from i to j (if there is an edge. If rGraph[i][j] is 0, then there is not) 
    for (u = 0; u < V; u++)
        for (v = 0; v < V; v++)
